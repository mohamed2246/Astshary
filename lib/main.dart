...
class PushNotificationService {
  ...
  final Completer<void> _splashCompleter = Completer<void>(); // Completer to control splash navigation
  ...
  Future<void> initialize(BuildContext context) async {
    ...
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) {
      ...
      if (!_notificationHandled) {
        ...
        isNotificationPressed = true; // Ensure this is set when a notification is pressed
  ...
  Future<void> handleNotification(RemoteMessage message, BuildContext context) async {
    ...
    if (message.notification != null) {
      ...
      await flutterLocalNotificationsPlugin.show(
        ...
        payload: jsonEncode(notification.toJson()),  // Ensure payload is correctly encoded
      );
    }

    // منع ظهور BottomSheet أخرى إذا كانت واحدة معروضة بالفعل
    if (notification.status == 'make_review' && !_isBottomSheetDisplayed && !_notificationHandled) {
  ...
  void _handleNotificationClick(BuildContext context, String payload) {
    ...
    if (payload.isNotEmpty) {
      ...
      if (data.status == 'make_review' && !_isBottomSheetDisplayed && !_notificationHandled) {
        ...
        _notificationHandled = true; // Confirm that the notification has been handled
      }
    } else {
    ...
    MultiBlocProvider(
      providers: [
        BlocProvider<ThemeCubit>(
          create: (BuildContext context) => ThemeCubit()..changeTheme(ThemeStateEnum.Initial),
        ),
...
*/
  ...
  final Completer<void> _splashCompleter = Completer<void>(); // Add Completer here
  ...
  void _handleIncomingLink(Uri uri) async {
    if (uri.queryParameters.containsKey('doctorId')) {
      ...
      Navigator.of(navigatorKey.currentState!.context).pushAndRemoveUntil(
        MaterialPageRoute(
          builder: (context) => SplashView(onComplete: () {
            // After the splash screen completes, navigate to the desired screen
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused) {
      ...
      if (!_pushNotificationService._isBottomSheetDisplayed && isNotificationPressed) {
        _checkInitialMessage();
  ...
  Future<void> _checkInitialMessage() async {
    ...
    if (initialMessage != null) {
      context.read<SpecialtiesCubit>().notificationReceived(initialMessage, false);
      isNotificationPressed = true; // Set this when the app is opened from a terminated state via notification
      ...
      NotificationData notification = NotificationData.fromJson(initialMessage.data);
          ...
          builder: (BuildContext context, state) {
            ...
            return MaterialApp(
              ...
              home: SplashView(onComplete: () {
                _splashCompleter.complete(); // Complete the completer when splash is done
              }),
