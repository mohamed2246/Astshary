...
import 'package:astshary/Feachers/ChooseSpecialtiesView/Data/models/AllSpecializationsViewModel.dart' as AllSpecializationsView ;
...
import '../../../../DoctorDetailsScreen/Presentation/view_models/DoctorDetailsModel.dart' as DoctorDetails;
import '../../../../FindDoctorView/Data/AllDoctorsSpeCityAreaModel.dart'as allDocSpeCiArea;
import '../../../../HomePageView/Data/AllRecomendedDoctors.dart' as recomendationDoctor;
...
class SpecialtiesCubit extends Cubit<SpecialtiesState> {
  ...
  Future<void> saveDataSharePref() async {
    ...
    print("this is the patient id i saved ${confirmation_model!.reservation!.patientId.toString()}");

    await sharedPreferencesService.setName(/*name*/  confirmation_model!.reservation.patient.name);
    await sharedPreferencesService.setPhone(/*phone*/confirmation_model!.reservation.patient.phone);
  ...
  Future<void> fetchCities() async {
    ...
    DioHelper.getData(
      ...
      final city.CitiesModel citiesModel = city.CitiesModel.fromJson(json.decode(value.data) );
...
*/
    ...
    DioHelper.getData(
      ...
      if (error is DioError) {
        if (error.response != null && error.response!.statusCode == 404) {
          ...
          SearchCityErrorResponse errorResponse = SearchCityErrorResponse.fromJson(decodedJson);
  ...
  void searchApiArea() {
    ...
    DioHelper.getData(
      url: 'area/search/${selectedCityId}?query=${AreaSearchControler.text.toString()}',
      token: 'Bearer $savedToken',
    ).then((response) {
      var decodedJson = json.decode(response.data);
  ...
  Future<void> fetchAreas() async {
    ...
    DioHelper.getData(
            url: 'city/areas/${selectedCityId}',
            token:
                'Bearer ${savedToken}')
        .then((value) async {
      final area.AreaModel areaModel = area.AreaModel.fromJson(json.decode(value.data));
...
*/
  ...
  List<Widget> MakeDoctorServicesList({required bool isArabic, required BuildContext context, required double screenHeight}) {
    List<Widget> list = [];
    ...
    for (int i = 0; i < doctorDetailsModel!.allServices!.length; i++) {
      ...
      list.add(Padding(
        ...
        child: Container(
          ...
          padding: EdgeInsets.symmetric(vertical: 10, horizontal: 8), // Add padding to container
          child: Text(
            '$serviceName',
  ...
  List<Widget> MakeClinicInsuranceList({required bool isArabic , required BuildContext context , required double screenHeight}){
   List<Widget> list =[] ;
   //context.locale.languageCode == 'ar'

   print('listOfInsurance[selectedBranchIndex] ${listOfInsurance[selectedBranchIndex]}');
  ...
  List<Widget> MakeDoctorInsuranceList({required bool isArabic , required BuildContext context , required double screenHeight}){
    List<Widget> list =[] ;
    //context.locale.languageCode == 'ar'
      ...
      if(!isArabic){
        ...
        list.add(Padding(
          ...
          child: Container(
            ...
            child: Text(
              '${doctorDetailsModel!.insurances![i]!.nameEn}',
              style:
              Theme.of(context).textTheme.bodyText2!.copyWith(
                color:Color(0xFF424242),
                fontFamily: itArabic
        ...
        list.add( Padding(
          ...
          child: Container(
            ...
            child: Text(
              '${doctorDetailsModel!.insurances![i]!.nameAr}',
              style:
              Theme.of(context).textTheme.bodyText2!.copyWith(
                color:Color(0xFF424242),
                fontFamily: itArabic
   ...
   /* SpecialListsArList.clear();
    ...
    DioHelper.getData(
      ...
      if (clinicModel.status.toString() == "200") {
        ...
        clinicNameEn = clinicModel.enterprise.nameEn ;
        print('clinicNameEn ${clinicNameEn}');
        clinicNameAr = clinicModel.enterprise.nameAr ;
        clinicLogo = clinicModel.enterprise.logo ;
  ...
  void fetchDoctorDetails() {
    ...
    DioHelper.getData(
      ...
      try {
        ...
        if (decodedJson != null && decodedJson['status'] == 200 && decodedJson['doctor'] != null) {
          print('Success: Fetched doctor details successfully.');
          doctorDetailsModel = DoctorDetails.Doctor.fromJson(decodedJson['doctor']);
          setSelectedBranchDoctorDetailsId(doctorDetailsModel!.branches![0]!.id);
    ...
    DioHelper.getData(
      ...
      try {
        ...
        if (decodedJson != null && decodedJson['status'] == 200 ) {
          ...
          allSepcialistsViewModel = AllSpecializationsView.SpecializationsResponse.fromJson(decodedJson);
          specializationsHomeList = List.from(allSepcialistsViewModel!.mostSelectedSpecializations!);
          if(specializationsHomeList!.length > 3){
            specializationsHomeList![3] =AllSpecializationsView.Specialization(icon: 'assets/images/more.svg', nameAr: 'المزيد', nameEn: 'More', id: 00, createdAt: DateTime.now(), updatedAt: DateTime.now(), isMostSelected: false) ;
          }else{
            specializationsHomeList!.add(AllSpecializationsView.Specialization(icon: 'assets/images/more.svg', nameAr: 'المزيد', nameEn: 'More', id: 00, createdAt: DateTime.now(), updatedAt: DateTime.now(), isMostSelected: false));
          }
...
List<SearchSpecModel> SpesialistesListSearch = [];
  ...
  void searchApiSpec() {
    ...
    DioHelper.getData(
      url: 'specializations/search?query=${SpecialistsSearchControler.text.toString()}',
      token: 'Bearer $savedToken',
    ).then((response) {
      var decodedJson = json.decode(response.data);
  ...
  void getDoctorsFromSpecCityArea(){
    ...
    DioHelper.getData(
        url: 'doctors/${selectedSpecificationId}/${selectedCityId}/${selectedAreaId??''}',
        token: 'Bearer ${savedToken}')
        .then((value) async {
      try {
        printLongString(value.data.toString());
        ...
        if (decodedJson != null) {
          ...
          doctorSpeCityAreaModel =  allDocSpeCiArea.DoctorResponse.fromJson(decodedJson);

          print("this is the avarible appointment ${doctorSpeCityAreaModel!.doctors[0].availableAppointments![0]!.date}");
...
*/
        ...
        if (decodedJson != null && decodedJson['status'] == 200 ) {
          print('Success: fetch All getDoctorsSlider successfully.');
          allRecomandationDoctorModel = recomendationDoctor. AllRecommendedDoctors.fromJson(decodedJson);

          print('getRecomendationDoctors list leanth ${getListDoctorSlider}');
...
////////////////////////// GET APPOINTMENTS/////////////////////////////////////////////
  ...
  /*void fetchMyAppointment({bool isRecadual = false ,var doctorRescadualId ,var interPriseRescadualId}) {
    nextDays = [];
    ...
    DioHelper.getData(
        ...
        'doctor/appointments/${doctorRescadualId}/${interPriseRescadualId}'
            :'doctor/appointments/${doctorDetailsModel!.id}/${doctorDetailsModel!.enterprise.id}',
        token: 'Bearer $savedToken')
        .then((value) async {
      var decodedJson = json.decode(value.data);
      ...
      if (myAppointMentModel!.status.toString() == "200") {
        ...
        // Get the first date with active appointments and the farthest appointment date
        if(!myAppointMentModel!.appointments!.isEmpty){
          // Filter appointments to find the earliest and farthest active appointment dates
          List<DateTime> activeAppointmentDates = myAppointMentModel!.appointments!
          ...
          if (activeAppointmentDates.isNotEmpty) {
            DateTime earliestActiveAppointmentDate = activeAppointmentDates.first;
            DateTime farthestAppointmentDate = activeAppointmentDates.last;

            // Generate the days list starting from the earliest active appointment date
            nextDays = generateDaysList(earliestActiveAppointmentDate, farthestAppointmentDate);
          }
        }
  ...
  List<Appointments> availableAppointmentsForSelectedDate() {
    return myAppointMentModel!.appointments!.where((appointment) {
      ...
      return appointmentDate.day == selectedDate.day && appointmentDate.month == selectedDate.month;
    }).toList();
  }
  ...
  List<String> availableTimesForSelectedDate() {
    ...
    return availableAppointmentsForSelectedDate().map((appointment) {
      DateTime time = DateTime.parse("${appointment.date!} ${appointment.time}");
...
////////////////////////// GET APPOINTMENTS/////////////////////////////////////////////
  ...
  void setInsuranceTextIdLogin(newValue){
    ...
    if(itArabic){
      ...
      index =  insuranceLoginAr.indexOf(selectedInsurance!);
    ...
    print("this is the e=selected insurance text ${selectedInsurance}  and id ${selectedLoginInsuranceId}");
 ...
 /* Future<void> pickImageFromGallery(BuildContext context) async {
  ...
  bool validateInsuranceDetails() {
    return selectedInsurance != null && InsuranceImage.isNotEmpty;
  }

  bool validateForm(BuildContext context) {
    if (!validateFullName(name1Controler.text)) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('fullNameEmpty'.tr())));
      return false;
    }
    if (!validatePhoneNumber(phone1Controler.text)) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('phoneInvalid'.tr())));
      return false;
    }
    if (isAnotherPerson) {
      if (!validateFullName(name2Controler.text)) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('otherFullNameEmpty'.tr())));
        return false;
      }
      if (!validatePhoneNumber(phone2Controler.text)) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('otherPhoneInvalid'.tr())));
        return false;
      }
    }
    if (isInsurance && !validateInsuranceDetails()) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('insuranceDetailsInvalid'.tr())));
      return false;
    }
    return true;
  }
...
////////////////////////// Book EndPOINT /////////////////////////////////////////////
  ...
  void postConfirmation(context) {
    ...
    print("insurance_id ${selectedLoginInsuranceId == null ? '' : selectedLoginInsuranceId.toString()}");
...
/*      "second_name": name2Controler.text.toString(),
    ...
    DioHelper.postData(
      ...
      if (confirmation_model!.message.toString() == "Appointment created successfully") {
        log("this is the confirmation_model $decodedJson");
        print('Suuuuuuuuuuuuuuuuuuuucess');

        selectedDoctorIdDetails = int.parse(confirmation_model!.reservation!.doctorId!);
/*
...
////////////////////////// My Appointments EndPOINT /////////////////////////////////////////////
  ...
  void cancleAppointmentRecervations(id){
    ...
    DioHelper.getData(
      ...
      if (cancleModel!.message.toString() ==  "تم الغاء الحجز بنجاح") {
        print("this is the cancleModel ${decodedJson}");
...
//,required String enterprise_id , required String patient_id ,required String doctor_id ,required bool is_insured  ,required String insurance_id ,required String insurance_card
  ...
  void RescadaulAppointmentRecervations({required String doctor_appointment_id }) {
    isMyAppointmentsRecervationsLoading = true;
    ...
    DioHelper.postData(
      ...
      if (rescadualModel!.message.toString() == "تم تحديث الحجز بنجاح" ||rescadualModel!.message.toString() == "تم إضافة الحجز الجديد بنجاح") {
        print("RescadaulAppointmentRecervations $decodedJson");
        print('Suuuuuuuuuuuuuuuuuuuucess');
...
//////////////////////////  Login Profile EndPOINT /////////////////////////////////////////////
  ...
  void postLoginProfileApi(context) {
    ...
    print("image ${InsuranceImage ?? ''}");
    print("gender ${selectedGender == 1 ? "female" : "male"}");
    print("selectedLoginInsuranceId ${selectedLoginInsuranceId == null ? '' : selectedLoginInsuranceId.toString()}");
    ...
    DioHelper.postData(
      ...
      if (decodedJson['message'] != null) {
        ...
        if (decodedJson['patient'] != null) {
          ...
          await sharedPreferencesService.setName(/*name1Controler.text.toString()*/successResponse!.patient!.name!);
          await sharedPreferencesService.setPhone(/*phone1Controler.text.toString()*/successResponse!.patient!.phone!);
          await sharedPreferencesService.setpatientId(successResponse!.patient!.id.toString());
          switchValue1 = successResponse!.patient!.notification_avaliable == "1" ? true : false ;
  ...
  Future<void> postLoginProfileApiFromSharedPref(context) async {
    ...
    try {
      ...
      if (decodedJson['message'] != null) {
        ...
        await sharedPreferencesService.setPhone(successResponse!.patient!.phone!);
        await sharedPreferencesService.setpatientId(successResponse!.patient!.id.toString());

        isAcceptNotifications = successResponse!.patient!.notification_avaliable == "1";
  ...
  void postNotificationOnOfApi(bool value) async {
    ...
    try {
      ...
      if (decodedJson['message'] != null) {
        ...
        changeNotifcationProfile = ChangeNotificationStatus.fromJson(decodedJson);
        switchValue1 = changeNotifcationProfile!.data!.notificationAvaliable=="1" ?true:false;
...
////////////////////////////////////////////// seach recent//////////////////////////////////////
  ...
  void SortSetselectedSpecialIndexSort(index){
    ...
    selectedSpecialSortvalue =  allSepcialistsViewModel!.specializations[index-1].nameAr;
...
////////////////////////////////////////////// search recomendation
  ...
  void fetchSearchDoctorsFilterRecoApi() {
    ...
    print('recoSearchControlerfindDoctor.text ${recoSearchControlerfindDoctor.text}');
    print('recoSearchControlerfindDoctor.text ${ getSpecializationNameArById( allSepcialistsViewModel!, selectedSpecificationId) }');
    print('gender ${selectedGenderSortvalue}');
    print('exp_years${selectedExperienceSortvalue}');
    print('rating ${selectedRatingSortvalue}');

    DioHelper.getData(
        url: 'recommended/doctors/search?name=${recoSearchControlerfindDoctor.text}&specialization=${selectedSpecialSortvalue}&gender=${selectedGenderSortvalue}&exp_years=${selectedExperienceSortvalue}&rating=${selectedRatingSortvalue}&page=1&pageSize=100',
        token: 'Bearer $savedToken'
    ).then((value) async {
      ...
      try {
        ...
        if (value.statusCode == 200) {
          if (decodedJson != null) {
            if (decodedJson['data'] is Map<String, dynamic>) {
              // Handle SearchHomeFilterDoctors response
              searchRecoFilterModel = SearchHomeFilterDoctors.fromJson(decodedJson);
              print('Success: fetch All Recoooooooooo searchRecoFilterModel successfully.');
              recomindationLoading = false;
              emit(fetchSearchDoctorsFilterHomeApiSuccess());
            } else if (decodedJson['data'] is List) {
              // Handle SearchHomeSpecModel response
              searchHomeSpecModel = SearchHomeSpecModel.fromJson(decodedJson);
              print('Success: Recoooooooooo fetch All searchHomeSpecModel successfully.');
              recomindationLoading = false;
              emit(fetchSearchDoctorsFilterHomeApiSuccess());
            } else if (decodedJson['message'] != null) {
              // Handle SearchHomeNoDataFound response
...
//////////////////////////////////////////////////////////////////////////////////search find doctor ///////////////
  ...
  String? getSpecializationNameArById(AllSpecializationsView.SpecializationsResponse response, var id) {
...
/*
  Future<void> getNotificationFromApi({bool isFromNotificationListScreen = false , BuildContext? context}) async {
              ...
              if(isFromNotificationListScreen == false) {
                ...
                if(!isNotificationPressed){
                  showModalBottomSheet(
                    ...
                    // إعادة تعيين المتغير إلى false عندما يتم إغلاق BottomSheet وتخزينه
  ...
  Future<void> getNotificationFromApi({bool isFromNotificationListScreen = false, BuildContext? context}) async {
    ...
    await DioHelper.getData(
      ...
      if (notifications_model!.status.toString() == "200") {
        ...
        bool actionExecuted = false; // Flag to ensure the action is performed only once
          ...
          if (element.isnew == "1" && !actionExecuted) {
            ...
            if (element.status == 'make_review') {
              if (!isFromNotificationListScreen) {
                bottomSheetDoctorId = element.doctor_id.toString();
                bottomSheetAppointmentId = element.doctor_appointment_id.toString();
                print("this is notification from getNotificationFromApi");
                print("isNotificationPressed getNotificationFromApi ${isNotificationPressed}");
...
*/
  ...
  void notificationReceived(RemoteMessage messagel, bool isPressed) async {
    ...
    await prefs.setString('notification_doctor_id', messagel.data['doctor_id'] ?? '');
