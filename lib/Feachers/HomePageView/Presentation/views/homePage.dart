...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    print('this is the image from the home ${imageClient.toString()}');
    MediaQueryData mediaQuery = MediaQuery.of(context);
    var hieght = MediaQuery.of(context).size.height;
    var width = MediaQuery.of(context).size.width;
    if(imageClient ==null || imageClient ==''){
      print('loooocal image');
    }else{
      print('global image');
                              ...
                              child: Row(
                                children: <Widget>[
                                  InkWell(
                                    ...
                                    child: CircleAvatar(
                                      backgroundImage: /*cubit.successResponse == null ||cubit.successResponse!.patient!.image == null
                                          ?*/ /*const AssetImage('assets/images/profile_image.png') as ImageProvider<Object>?*/
                                          /*:*/ imageClient ==null || imageClient =='' ||imageClient =="null" ? AssetImage('assets/images/profile_image.png'):NetworkImage(imageClient!) as ImageProvider<Object>,
                                      radius: 24.0,
                                    ),
                                  ),
                                  SizedBox(width: 12.0),
...
String translateDayToEnglishAbbreviation(String arabicDay) {
...
String translateDayToArabicAbbreviation(String arabicDay) {
  ...
  return dayAbbreviations[arabicDay] ?? 'Invalid day';
}

String translateMonthToEnglish(String arabicMonth) {
  ...
  return monthTranslations[arabicMonth] ?? 'Invalid month';
}

String translateMonthToArabic(String arabicMonth) {
  Map<String, String> monthTranslations = {
    '01': 'يناير',
    '02': 'فبراير',
    '03': 'مارس',
...
String getMonthFromDate(String date) {
  // Assuming the date is in the format 'YYYY-MM-DD'
  List<String> dateParts = date.split('-');
  return dateParts.length == 3 ? dateParts[1] : '';
}
...
String formatTime(String time, bool isArabic) {
  ...
  if (timeParts.length == 3) {
    ...
    String period = isArabic ? 'ص' : 'AM';  // Default to AM in both languages

    if (hour == 0) {
      hour = 12;  // Midnight case
      period = isArabic ? 'ص' : 'AM';
    } else if (hour >= 12) {
      if (hour > 12) hour -= 12;  // Convert 24-hour format to 12-hour format
      period = isArabic ? 'م' : 'PM';  // Set to PM in both languages
    }
